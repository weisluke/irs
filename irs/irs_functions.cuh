...
\return d2_alpha_star_d_zbar2 = 2 * theta^2 * sum(m_i / (z - z_i)_bar^3)
******************************************************************************/
template <typename T>
__device__ Complex<T> d2_star_deflection_d_zbar2(Complex<T> z, T theta, star<T>* stars, TreeNode<T>* node)
{
	...
	******************************************************************************/
	for (int i = 0; i < node->numstars; i++)
	{
		d2_alpha_star_bar_d_z2 += stars[node->stars + i].mass / ((z - stars[node->stars + i].position) * (z - stars[node->stars + i].position) * (z - stars[node->stars + i].position));
	}
	for (int j = 0; j < node->numneighbors; j++)
	{
		TreeNode<T>* neighbor = node->neighbors[j];
		for (int i = 0; i < neighbor->numstars; i++)
		{
			d2_alpha_star_bar_d_z2 += stars[neighbor->stars + i].mass / ((z - stars[neighbor->stars + i].position) * (z - stars[neighbor->stars + i].position) * (z - stars[neighbor->stars + i].position));
		}
	}
...
\return d2_alpha_local_d_zbar2 = theta^2 * sum(i * (i - 1) * (i - 2) * a_i 
                                 * (z - z_0) ^ (i - 3))
		                         where a_i are coefficients of the lensing
                                 potential in units of the node size
******************************************************************************/
...
{
	Complex<T> d2_alpha_local_bar_d_z2;
	Complex<T> dz = (z - node->center) / node->half_length;

	for (int i = node->expansion_order; i >= 3; i--)
	{
		d2_alpha_local_bar_d_z2 *= dz;
		d2_alpha_local_bar_d_z2 += node->local_coeffs[i] * i * (i - 1) * (i - 2);
	}
	d2_alpha_local_bar_d_z2 *= (theta * theta);
	/******************************************************************************
	account for node size
	******************************************************************************/
	d2_alpha_local_bar_d_z2 /= (node->half_length * node->half_length * node->half_length);

	return d2_alpha_local_bar_d_z2.conj();
}
...
/******************************************************************************
gradient of the magnification at a point in the image plane
...
******************************************************************************/
template <typename T>
__device__ Complex<T> grad_magnification(Complex<T> z, T kappa, T gamma, T theta, star<T>* stars, T kappastar, TreeNode<T>* node,
	int rectangular, Complex<T> corner, int approx, int taylor_smooth)
{
	T mu = magnification(z, kappa, gamma, theta, stars, kappastar, node, rectangular, corner, approx, taylor_smooth);
	...
	Complex<T> d2_w_d_zbar2 = -d2_alpha_star_d_zbar2 - d2_alpha_local_d_zbar2 - d2_alpha_smooth_d_zbar2;

	T d_mu_d_y1 = -mu * mu * 
		(-2 * (d_w_d_zbar * d2_w_d_zbar2.conj()).re * (1 / d_w_d_z + 1 / d_w_d_zbar).re 
		+ 2 * (d_w_d_zbar * d2_w_d_zbar2.conj()).im * (1 / d_w_d_zbar - 1 / d_w_d_z).im);

	T d_mu_d_y2 = -mu * mu *
		(-2 * (d_w_d_zbar * d2_w_d_zbar2.conj()).re * -(1 / d_w_d_z + 1 / d_w_d_zbar).im
			+ 2 * (d_w_d_zbar * d2_w_d_zbar2.conj()).im * (1 / d_w_d_z - 1 / d_w_d_zbar).re);

	return Complex<T>(d_mu_d_y1, d_mu_d_y2);
}
...
__global__ void shoot_rays_kernel(T kappa, T gamma, T theta, star<T>* stars, T kappastar, TreeNode<T>* root, int num_rays_factor, 
	int rectangular, Complex<T> corner, int approx, int taylor_smooth,
	Complex<T> hlx, Complex<int> numrayblocks, T hly, T* pixmin, T* pixsad, T* pixels, int npixels, int* percentage)
{
	...
	for (int l = blockIdx.y; l < numrayblocks.im; l += gridDim.y)
	{
		for (int k = blockIdx.x; k < numrayblocks.re; k += gridDim.x)
		{
			...
			Complex<T> w;
			T mu;
			T area_img = 4 * ray_half_sep.re * ray_half_sep.im * npixels * npixels / (2 * hly * 2 * hly);
			T to_add;
			for (int j = threadIdx.y; j < num_rays; j += blockDim.y)
			{
				for (int i = threadIdx.x; i < num_rays; i += blockDim.x)
				{
					...
					******************************************************************************/
					...
					mu = magnification(z, kappa, gamma, theta, tmp_stars, kappastar, node, rectangular, corner, approx, taylor_smooth);
					...
					******************************************************************************/
					...
					if (pixmin && pixsad)
					{
						if (mu >= 0)
						{
							atomicAdd(&pixmin[ypix.im * npixels + ypix.re], to_add);
						}
						else
						{
							atomicAdd(&pixsad[ypix.im * npixels + ypix.re], to_add);
						}
					}
					atomicAdd(&pixels[ypix.im * npixels + ypix.re], to_add);
...
******************************************************************************/
template <typename T>
__global__ void initialize_pixels_kernel(T* pixels, int npixels)
{
